...
private let helperName = "CrossHands Computer Use"
private let helperBundleId = "ai.crosshands.ComputerUse"
...
final class Provider {
    ...
    private func rememberSnapshot(
        ...
        let snapshotIdKey = "snapshot-id:\(cachedSnapshot.id.lowercased())"
        snapshots[snapshotIdKey] = cachedSnapshot
        storedKeys.append(snapshotIdKey)
    ...
    private func cachedSnapshot(params: [String: JSONValue]) throws -> Snapshot? {
        if let snapshotId = params["snapshotId"]?.string, !snapshotId.isEmpty {
            return snapshots["snapshot-id:\(snapshotId.lowercased())"]
        }
    ...
    private func renderListedApp(_ app: AppDescriptor) -> [String: Any] {
        [
            ...
            "pid": Int(app.pid),
            "processStartedAt": ISO8601DateFormatter().string(from: app.processStartedAt),
            "executableId": app.executableId,
            "isRunning": true,
            "lastUsedAt": NSNull(),
            "useCount": NSNull(),
        ]
    ...
    private func providerHandshake() -> [String: Any] {
        [
            ...
            "providerProtocol": providerProtocolVersion,
            "publicContract": publicContractVersion,
    ...
    private func providerCapabilities() -> [String: Any] {
        [
            "platform": "darwin",
            "provider": providerName,
    ...
    private func permissionStates() -> [String: String] {
        [
            "accessibility": accessibilityTrusted() ? "granted" : "denied",
            "screenshots": screenCaptureTrusted() ? "granted" : "denied",
        ]
    }
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrusted() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for CrossHands Computer Use. Run `crosshands permissions` or open System Settings > Privacy & Security > Accessibility, grant access to CrossHands Computer Use, then retry."
            )
        }
        ...
        return [
            "snapshot": [
                ...
                "app": [
                    ...
                    "processStartedAt": ISO8601DateFormatter().string(from: snapshot.app.processStartedAt),
                    "executableId": snapshot.app.executableId,
                ],
                "window": [
                    "id": Int(snapshot.windowId),
                    "ownerPid": Int(snapshot.app.pid),
                    "index": WindowCapture.candidates(pid: snapshot.app.pid)
                        .filter { $0.layer == 0 }
                        .firstIndex { $0.windowId == snapshot.windowId } ?? 0,
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    let permissionHint = visibleWindowCount > 0
        ? " The app has visible windows, so macOS Accessibility may need CrossHands Computer Use toggled off and on again in System Settings."
        : ""
...
private func snapshotNamespace(_ params: [String: JSONValue]) -> String {
    if let context = params["interactionContext"]?.string, !context.isEmpty {
...
private final class AXSnapshotReader {
    ...
    func valueString(_ element: AXUIElement, role: String) -> String? {
        if isSecureTextElement(element, role: role) {
            return SecureFieldMinimization.value(nil, isSecure: true)
        }
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        captureImageWithScreenCaptureKit(windowId: windowId, bounds: bounds).map {
            CapturedImage(image: $0, engine: "screenCaptureKit")
        }
    }
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable CrossHands Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag CrossHands Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag CrossHands Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let title = label(ready ? "Computer Use is Ready" : "Enable CrossHands Computer Use", size: 22, weight: .bold)
        let subtitle = label(
            ready ? "CrossHands can use local apps when you ask." : "Grant permissions so CrossHands can use apps when you ask.",
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "crosshands-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag CrossHands Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "crosshands-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "CrossHands Computer Use")
...
private final class SocketListener: @unchecked Sendable {
    ...
    private func handleConnection(_ fd: Int32) {
        defer { close(fd) }
        let authorizedPeer = isAuthorizedLocalPeer(fd)
...
private func isAuthorizedLocalPeer(_ fd: Int32) -> Bool {
    var peerUser = uid_t.max
    var peerGroup = gid_t.max
    guard getpeereid(fd, &peerUser, &peerGroup) == 0 else {
        return false
    }
    return peerUser == geteuid()
}
...
private func runStdio() {
    fputs("CrossHands Computer Use provider must be launched by CrossHands in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: crosshands-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("crosshands-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: crosshands-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
