...
describe('set product version', () => {
  ...
  it('rewrites registry files to 0.2.0 and is idempotent', async () => {
    ...
    const result = await setProductVersion({ root, version: '0.2.0' })
    ...
    const again = await setProductVersion({ root, version: '0.2.0' })
